...
import {
  listStaffBillsForMonitorApi,
  type StaffBillRow,
} from "../../../api/staff/staff.bills";
...
export default function MonitorWaiterBillsPage() {
  ...
  useEffect(() => { donePresetRef.current = donePreset; }, [donePreset]);
  useEffect(() => { doneRangeRef.current = doneRange; }, [doneRange]);
  ...
  async function loadBills(opts?: { silent?: boolean; goPage?: number }) {
    ...
    try {
      ...
      const res = await listStaffBillsForMonitorApi(params);
  ...
  useEffect(() => {
    ...
    socket.on("bill.cancelled", refreshBills);
